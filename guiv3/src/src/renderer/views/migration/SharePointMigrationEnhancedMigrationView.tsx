...
export default function SharePointMigrationEnhancedMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useSharePointMigrationEnhancedMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="share-point-migration-enhanced-migration-view" data-cy="share-point-migration-enhanced-migration-view">
      <h1 className="text-xl font-semibold">SharePointMigrationEnhanced Migration</h1>
