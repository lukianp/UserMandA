...
export default function ScheduledTaskDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useScheduledTaskDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="scheduled-task-discovery-view" data-cy="scheduled-task-discovery-view">
      <h1 className="text-xl font-semibold">ScheduledTask Discovery</h1>
