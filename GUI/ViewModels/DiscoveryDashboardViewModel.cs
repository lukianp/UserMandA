...
namespace MandADiscoverySuite.ViewModels
{
    ...
    public class DiscoveryDashboardViewModel : BaseViewModel
    {
        ...
        private void LoadModulesFromRegistry(ModuleRegistry registry)
        {
            ...
            foreach (var kvp in registry.Modules)
            {
                ...
                if (moduleInfo.FilePath?.StartsWith("Discovery/", StringComparison.OrdinalIgnoreCase) == true)
                {
                    ...
                    module.DisplayName = moduleInfo.DisplayName ?? moduleId;
                    module.Description = moduleInfo.Description ?? "Discovery module";
        ...
        {
            // Create basic fallback modules
            var moduleNames = new[]
            {
                "ActiveDirectoryDiscovery", "AzureDiscovery", "AzureResourceDiscovery",
                "ExchangeDiscovery", "TeamsDiscovery", "SharePointDiscovery",
                "IntuneDiscovery", "NetworkInfrastructureDiscovery", 
                "SQLServerDiscovery", "FileServerDiscovery"
            };
            
            var fallbackModules = moduleNames.Select(name => 
            {
                var module = new DiscoveryModuleViewModel(_mainViewModel);
                module.DisplayName = name.Replace("Discovery", " Discovery");
                module.Description = $"Discovery module for {name}";
        ...
        private void SubscribeToCollectionChanges()
        {
            ...
            if (_mainViewModel.Computers is INotifyCollectionChanged computersCollection)
            {
                computersCollection.CollectionChanged += (s, e) => UpdateCounts();
            }
