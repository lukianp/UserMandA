...
function Get-GPOData {
    ...
    try {
        Write-MandALog "ðŸŽ¯ Starting comprehensive GPO analysis..." -Level "HEADER"
        ...
        if (!(Test-Path $gpoReportsPath)) {
            ...
            Write-MandALog "ðŸ“ Created GPO reports directory: $gpoReportsPath" -Level "INFO"
        }
        
        # Get all GPOs in domain with enhanced error handling
        try {
            $gpos = Get-GPO -All -Server $DomainController -ErrorAction Stop
            Write-MandALog "ðŸ“Š Found $($gpos.Count) GPOs to analyze" -Level "INFO"
        } catch {
            Write-MandALog "âŒ Failed to retrieve GPOs: $($_.Exception.Message)" -Level "ERROR"
            
            # Try alternative method using PowerShell AD module
            try {
                Write-MandALog "ðŸ”„ Attempting alternative GPO retrieval method..." -Level "PROGRESS"
                ...
                Write-MandALog "ðŸ“Š Retrieved $($gpos.Count) GPOs using AD module" -Level "SUCCESS"
            } catch {
                Write-MandALog "âŒ Alternative GPO retrieval also failed: $($_.Exception.Message)" -Level "ERROR"
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        Write-MandALog "âš ï¸ Standard GPO report failed for $($gpo.DisplayName): $($_.Exception.Message)" -Level "WARN"
                        ...
                        try {
                            ...
                            Write-MandALog "âš ï¸ Named GPO report also failed for $($gpo.DisplayName)" -Level "WARN"
                            
                            # Method 3: Create minimal XML structure
                            $xmlContent = New-MinimalGPOXML -GPO $gpo
                            $xmlContent | Set-Content -Path $reportPath -Encoding UTF8
                        }
                    }
                    
                    if ([string]::IsNullOrWhiteSpace($xmlContent)) {
                        Write-MandALog "âš ï¸ Empty XML report for GPO: $($gpo.DisplayName)" -Level "WARN"
                        continue
                    }
                    
                    # Parse XML with robust namespace handling
                    $gpoData = ConvertFrom-GPOXMLReportEnhanced -XmlContent $xmlContent -GPO $gpo
                    ...
                    Write-MandALog "âš ï¸ Error processing XML for GPO '$($gpo.DisplayName)': $($_.Exception.Message)" -Level "WARN"
                    $failedProcessing++
                }
                
            } catch {
                Write-MandALog "âŒ Error processing GPO '$($gpo.DisplayName)': $($_.Exception.Message)" -Level "ERROR"
                $failedProcessing++
            }
        }
        ...
        Write-MandALog "ðŸŽ¯ GPO Analysis Summary:" -Level "SUCCESS"
        Write-MandALog "  ðŸ“Š Total GPOs processed: $($allGPOs.Count)" -Level "INFO"
        Write-MandALog "  âœ… Successfully analyzed: $successfullyProcessed" -Level "SUCCESS"
        Write-MandALog "  âš ï¸ Failed to analyze: $failedProcessing" -Level "WARN"
        Write-MandALog "  ðŸ—‚ï¸ Drive mappings found: $($allDriveMappings.Count)" -Level "INFO"
        Write-MandALog "  ðŸ–¨ï¸ Printer mappings found: $($allPrinterMappings.Count)" -Level "INFO"
        Write-MandALog "  ðŸ“ Folder redirections found: $($allFolderRedirections.Count)" -Level "INFO"
        Write-MandALog "  ðŸ“œ Logon scripts found: $($allLogonScripts.Count)" -Level "INFO"
        ...
        Write-MandALog "âŒ GPO analysis failed: $($_.Exception.Message)" -Level "ERROR"
        throw
    }
}

function ConvertFrom-GPOXMLReportEnhanced {
    param(
        [string]$XmlContent,
        [object]$GPO
    )
    ...
    try {
        ...
        try {
            ...
            Write-MandALog "âš ï¸ Failed to parse XML for GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "WARN"
            
            # Try to clean and repair XML
            $cleanedXml = Repair-GPOXML -XmlContent $XmlContent
            try {
                $gpoXml = [xml]$cleanedXml
                Write-MandALog "âœ… Successfully repaired XML for GPO $($GPO.DisplayName)" -Level "SUCCESS"
            } catch {
                Write-MandALog "âŒ Unable to repair XML for GPO $($GPO.DisplayName)" -Level "ERROR"
                return $result
            }
        }
        ...
        Write-MandALog "ðŸ” Analyzing GPO: $($GPO.DisplayName)" -Level "DEBUG"
        ...
        Write-MandALog "âŒ Error parsing XML for GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "ERROR"
    }
...
function Repair-GPOXML {
    ...
    try {
        ...
        Write-MandALog "âš ï¸ XML repair failed: $($_.Exception.Message)" -Level "WARN"
        return $XmlContent
    }
}
        ...
        foreach ($xpath in $xpathExpressions) {
            try {
                ...
                if ($driveNodes -and $driveNodes.Count -gt 0) {
                    Write-MandALog "  ðŸ—‚ï¸ Found $($driveNodes.Count) drive mappings in $($GPO.DisplayName)" -Level "INFO"
                ...
                Write-MandALog "ðŸ” XPath '$xpath' failed for drive mappings: $($_.Exception.Message)" -Level "DEBUG"
                continue
            }
        }
    } catch {
        Write-MandALog "âŒ Error finding drive mappings in GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "WARN"
    }
        ...
        foreach ($xpath in $xpathExpressions) {
            try {
                ...
                if ($printerNodes -and $printerNodes.Count -gt 0) {
                    Write-MandALog "  ðŸ–¨ï¸ Found $($printerNodes.Count) printer mappings in $($GPO.DisplayName)" -Level "INFO"
                ...
                Write-MandALog "ðŸ” XPath '$xpath' failed for printer mappings: $($_.Exception.Message)" -Level "DEBUG"
                continue
            }
        }
    } catch {
        Write-MandALog "âŒ Error finding printer mappings in GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "WARN"
    }
        ...
        foreach ($xpath in $xpathExpressions) {
            try {
                ...
                if ($folderNodes -and $folderNodes.Count -gt 0) {
                    Write-MandALog "  ðŸ“ Found $($folderNodes.Count) folder redirections in $($GPO.DisplayName)" -Level "INFO"
                ...
                Write-MandALog "ðŸ” XPath '$xpath' failed for folder redirections: $($_.Exception.Message)" -Level "DEBUG"
                continue
            }
        }
    } catch {
        Write-MandALog "âŒ Error finding folder redirections in GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "WARN"
    }
        ...
        foreach ($xpath in $xpathExpressions) {
            try {
                ...
                if ($scriptNodes -and $scriptNodes.Count -gt 0) {
                    Write-MandALog "  ðŸ“œ Found $($scriptNodes.Count) logon scripts in $($GPO.DisplayName)" -Level "INFO"
                ...
                Write-MandALog "ðŸ” XPath '$xpath' failed for logon scripts: $($_.Exception.Message)" -Level "DEBUG"
                continue
            }
        }
    } catch {
        Write-MandALog "âŒ Error finding logon scripts in GPO $($GPO.DisplayName): $($_.Exception.Message)" -Level "WARN"
    }
    
    return $logonScripts
}

function New-MinimalGPOXML {
    param([object]$GPO)
