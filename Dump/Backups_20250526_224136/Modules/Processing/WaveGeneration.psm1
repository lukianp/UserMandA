...
function New-MigrationWaves {
    ...
    try {
        ...
        if ($generateByDepartment) {
            $migrationWaves = New-WavesByDepartment -Profiles $Profiles -MaxWaveSize $maxWaveSize -Configuration $Configuration
        } else {
            $migrationWaves = New-WavesByComplexity -Profiles $Profiles -MaxWaveSize $maxWaveSize -Configuration $Configuration
        }
        ...
        $waveSummary = New-WaveSummary -Waves $optimizedWaves
...
function New-WavesByDepartment {
    param(
        [array]$Profiles,
        [int]$MaxWaveSize,
        [hashtable]$Configuration
...
function New-WavesByComplexity {
    param(
        [array]$Profiles,
        [int]$MaxWaveSize,
        [hashtable]$Configuration
...
function Split-UsersIntoWaves {
    ...
    foreach ($user in $Users) {
        ...
        if ($currentWaveUsers.Count -ge $MaxWaveSize) {
            ...
            $wave = New-MigrationWave -Users $currentWaveUsers -Name $waveName
            $waves += $wave
...
function Optimize-MigrationWaves {
    ...
    try {
        ...
        $rebalancedWaves = Update-Waves -Waves $optimizedWaves -MaxWaveSize $maxWaveSize
    ...
    foreach ($smallWave in $sortedSmallWaves) {
        ...
        if (($currentMergeUsers.Count + $wave.TotalUsers) -le $MaxWaveSize) {
            ...
            if ($currentMergeUsers.Count -gt 0) {
                $mergedWave = New-MigrationWave -Users $currentMergeUsers -Name $currentMergeName
...
Export-ModuleMember -Function @(
    'New-MigrationWaves',
    'New-WavesByDepartment',
    'New-WavesByComplexity',
    'New-MigrationWave',
