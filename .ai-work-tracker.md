# AI Work Tracker - Last Updated: 2025-12-23 (Session Active)

## Current Task
**Goal:** Fix TieredExplorer Category-Aware Grouping
**User Request Summary:** TieredExplorer was grouping Users into "Applications" and Groups into "Platforms" because it only considered EntityType, not category
**Status:** COMPLETED - Fixed by adding category-aware grouping

### Fix Applied (2025-12-23):
1. **tieredExplorerTypes.ts** - Added:
   - `CATEGORY_TO_GROUP` mapping for category-based grouping (User→users, Group→groups, etc.)
   - `getGroupKeyForNode()` function that checks category first, then falls back to EntityType
   - Additional colors for 'interfaces' and 'company' groups

2. **useOrganisationExplorerLogic.ts** - Updated:
   - Changed import from `ENTITY_TYPE_TO_GROUP` to `getGroupKeyForNode`
   - `buildRelationGroups()` now uses `getGroupKeyForNode(node.type, node.metadata?.category)`
   - Added debug logging for group distribution
   - Updated GROUP_LABELS for clearer display names

### Result:
- Users (EntityType='application', category='User') now grouped into "Users & People"
- Groups (EntityType='platform', category='Group') now grouped into "Groups & Teams"
- Proper relation group categorization based on discovered data categories

## Parked Task: Application Fact Sheet System
**Goal:** Transform discovered applications into LeanIX-style Application Fact Sheets
**Status:** PARKED - Waiting for TieredExplorer fix
**Location:** Super prompt provided in conversation
**Summary:**
- Domain Model: ApplicationFact, ApplicationObservation, FieldProvenance, FactRelation, MigrationPlan
- Backend Service: ApplicationFactsService with CRUD, provenance tracking, relation management
- IPC Handlers: 7+ new endpoints for fact sheet operations
- UI Components: ApplicationFactSheet with 7 collapsible sections (Base Info, Lifecycle, Business, Technical, Security, Relations, Migration)
- Integration: Connect fact sheets with TieredExplorer for drilldowns
- Data Migration: Migrate existing CSV data to fact model
...
**Status:** COMPLETED - Core Components Complete

## TieredExplorer Implementation (Today)
**Goal:** Replace Sankey diagram with LeanIX-style column-based relations explorer
**Status:** IMPLEMENTED - But 0 relations showing (debugging)

### Files Created:
1. `tieredExplorerTypes.ts` - ExplorerNode, RelationGroup, ExplorerColumn, ExplorerState types
2. `TieredExplorer.tsx` - D3.js column-based renderer with curved bezier links
3. `useOrganisationExplorerLogic.ts` - State management hook for column navigation
4. `DetailPanel.tsx` - Right-side entity details panel

### Files Modified:
1. `OrganisationMapView.tsx` - Replaced SankeyDiagram with TieredExplorer
2. `ipcHandlers.ts` - Fixed get-discovery-files to skip test/old directories (rawold, testdata, logs, temp, old, test, demo)

### Known Issue:
- TieredExplorer shows "Relations: 0" for selected entities
- Added debug logging to identify if link source/target IDs match node IDs
- Console should show: `[Explorer] Building relationships from X links` and sample IDs

### Debug Logs Added:
- `useOrganisationExplorerLogic.ts` line 80-104: Logs link samples and relationship map sizes
- `useOrganisationExplorerLogic.ts` line 140-142: Logs when no relations found for entity
- `OrganisationMapView.tsx` line 140: Logs selected root entity with relation count
