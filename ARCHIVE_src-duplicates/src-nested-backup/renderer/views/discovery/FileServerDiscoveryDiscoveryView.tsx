...
import { useFileServerDiscoveryDiscovery } from "../../hooks/useFileServerDiscoveryDiscovery";
import DataTable from "../../components/DataTable";

export default function FileServerDiscoveryDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useFileServerDiscoveryDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="file-server-discovery-discovery-view" data-cy="file-server-discovery-discovery-view">
      <h1 className="text-xl font-semibold">FileServerDiscovery Discovery</h1>
