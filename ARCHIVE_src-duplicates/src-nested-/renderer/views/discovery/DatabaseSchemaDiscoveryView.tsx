...
import { useDatabaseSchemaDiscovery } from "../../hooks/useDatabaseSchemaDiscovery";
import DataTable from "../../components/DataTable";

export default function DatabaseSchemaDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useDatabaseSchemaDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="database-schema-discovery-view" data-cy="database-schema-discovery-view">
      <h1 className="text-xl font-semibold">DatabaseSchema Discovery</h1>
