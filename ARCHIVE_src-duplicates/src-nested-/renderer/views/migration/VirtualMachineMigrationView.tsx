...
import { useVirtualMachineMigration } from "../../hooks/useVirtualMachineMigration";
import DataTable from "../../components/DataTable";

export default function VirtualMachineMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useVirtualMachineMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="virtual-machine-migration-view" data-cy="virtual-machine-migration-view">
      <h1 className="text-xl font-semibold">VirtualMachine Migration</h1>
