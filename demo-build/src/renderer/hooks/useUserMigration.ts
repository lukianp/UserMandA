...
export function useUserMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.planMigration({ provider: "User", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.executeMigration({ provider: "User", profileId, args });
    setRunId(runId);
  }, [profileId]);
