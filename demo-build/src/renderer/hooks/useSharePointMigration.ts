...
export function useSharePointMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.planMigration({ provider: "SharePoint", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.executeMigration({ provider: "SharePoint", profileId, args });
    setRunId(runId);
  }, [profileId]);
