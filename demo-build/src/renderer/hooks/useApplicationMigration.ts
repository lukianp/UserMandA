...
export function useApplicationMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.planMigration({ provider: "Application", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.executeMigration({ provider: "Application", profileId, args });
    setRunId(runId);
  }, [profileId]);
