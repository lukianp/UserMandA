...
export function useMailboxMigrationBackupMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.planMigration({ provider: "MailboxMigrationBackup", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.executeMigration({ provider: "MailboxMigrationBackup", profileId, args });
    setRunId(runId);
  }, [profileId]);
