/**
 * Azure Functions Discovery Hook
 *
 * Event-driven hook for Azure Function Apps discovery operations.
 * Part of AzureHound-inspired enhancements for comprehensive Azure enumeration.
 *
 * Discovers:
 * - Function Apps with runtime and version info
...
export interface AzureFunctionsDiscoveryConfig {
  timeout?: number;
}

export interface AzureFunctionsDiscoveryProgress {
  percentage: number;
  message: string;
}

export const useAzureFunctionsDiscoveryLogic = () => {
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any>(null);
  const [progress, setProgress] = useState<AzureFunctionsDiscoveryProgress>({ percentage: 0, message: '' });
  ...
  const [config] = useState<AzureFunctionsDiscoveryConfig>({
    timeout: 300000,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeComplete = window.electron?.onDiscoveryComplete?.((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const functionApps = resultData?.FunctionApps || resultData?.functionApps || [];
        const count = Array.isArray(functionApps) ? functionApps.length : 0;
        ...
        addResult({
          id: `azure-functions-${Date.now()}`,
          name: 'Azure Functions Discovery',
          ...
          createdAt: new Date(),
          success: true,
          summary: `Discovered ${count} Function Apps`,
          errorMessage: '',
          additionalData: { functionApps },
        });

        setLogs((prev) => [...prev, { timestamp: new Date().toISOString(), message: `Discovery completed! Found ${count} Function Apps.`, level: 'success' }]);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    const token = `azure-functions-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    setIsDiscovering(true);
    setIsCancelling(false);
    setError(null);
    setLogs([{ timestamp: new Date().toISOString(), message: 'Starting Function Apps discovery...', level: 'info' }]);
