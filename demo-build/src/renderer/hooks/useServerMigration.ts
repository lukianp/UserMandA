...
export function useServerMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.planMigration({ provider: "Server", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.executeMigration({ provider: "Server", profileId, args });
    setRunId(runId);
  }, [profileId]);
