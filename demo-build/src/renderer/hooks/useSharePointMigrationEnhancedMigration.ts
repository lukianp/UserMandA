...
export function useSharePointMigrationEnhancedMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.planMigration({ provider: "SharePointMigrationEnhanced", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.migration.executeMigration({ provider: "SharePointMigrationEnhanced", profileId, args });
    setRunId(runId);
  }, [profileId]);
  ...
  useEffect(() => {
    ...
    window.electronAPI.migration.onMigrationProgress(onP);
    window.electronAPI.migration.onMigrationResult(onR);
  }, [runId]);
