...
import { useVirtualizationDiscovery } from "../../hooks/useVirtualizationDiscovery";
import DataTable from "../../components/DataTable";

export default function VirtualizationDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useVirtualizationDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="virtualization-discovery-view" data-cy="virtualization-discovery-view">
      <h1 className="text-xl font-semibold">Virtualization Discovery</h1>
