...
import { useSQLServerDiscoveryDiscovery } from "../../hooks/useSQLServerDiscoveryDiscovery";
import DataTable from "../../components/DataTable";

export default function SQLServerDiscoveryDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useSQLServerDiscoveryDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="s-q-l-server-discovery-discovery-view" data-cy="s-q-l-server-discovery-discovery-view">
      <h1 className="text-xl font-semibold">SQLServerDiscovery Discovery</h1>
