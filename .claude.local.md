t Claude Code Local Notes - UserMandA Project
...
**IMPORTANT**: The app MUST be built and run from `C:\enterprisediscovery\`, NOT from `D:\Scripts\UserMandA\guiv2\`
...
# 2. COPY modified source files from workspace to deployment
# (Only copy files you've changed, or copy all with -Force)
Copy-Item 'D:\Scripts\UserMandA\guiv2\src\renderer\components\molecules\*.tsx' `
  'C:\enterprisediscovery\src\renderer\components\molecules\' -Force -ErrorAction SilentlyContinue

Copy-Item 'D:\Scripts\UserMandA\guiv2\src\renderer\hooks\*.ts' `
  'C:\enterprisediscovery\src\renderer\hooks\' -Force -ErrorAction SilentlyContinue

Copy-Item 'D:\Scripts\UserMandA\guiv2\src\renderer\views\discovery\*.tsx' `
  'C:\enterprisediscovery\src\renderer\views\discovery\' -Force -ErrorAction SilentlyContinue

# 3. COPY PowerShell modules (REQUIRED for discovery to work)
...
# 4. BUILD from deployment directory
cd C:\enterprisediscovery

# CRITICAL: Must build BOTH main and renderer processes!
npm run build  # Builds main process (backend/Electron)

# Build renderer process (React UI) - REQUIRED for UI changes to appear
npx webpack --config webpack.renderer.config.js --output-path .webpack/renderer/main_window
...
# Rebuild BOTH processes (source changes won't appear without this!)
npm run build  # Main process
npx webpack --config webpack.renderer.config.js --output-path .webpack/renderer/main_window  # Renderer

# Restart
npm start
```

#### ⚠️ CRITICAL - Why UI Changes Don't Appear

**Electron apps have TWO separate build processes:**

1. **Main Process** (`npm run build`) - Backend/Node.js code
   - Location: `src/main/`, `src/index.ts`
   - Output: `.webpack/main/main.js`

2. **Renderer Process** (webpack renderer) - React UI code
   - Location: `src/renderer/` (components, views, hooks)
   - Output: `.webpack/renderer/main_window/`
   - **THIS IS WHAT YOU SEE IN THE APP!**

**If you only run `npm run build`, the UI changes won't appear because the renderer wasn't rebuilt!**

Always build BOTH when changing UI code (renderer) OR update package.json to include both in the build script.
...
## Discovery View State Persistence Pattern

### Problem
Discovery results were not persisting when navigating away from and back to a discovery view. Each time the component remounted, results were reset to empty.

### Solution
Implement state restoration from the global discovery store on component mount.

**Step 1: Add getter to Discovery Store** (`src/renderer/store/useDiscoveryStore.ts`)
...
**Step 2: Update Discovery Hook** (e.g., `src/renderer/hooks/useAzureDiscoveryLogic.ts`)

```typescript
// Import the getter
const { addResult, setProgress, getResultsByModuleName } = useDiscoveryStore();

// Add useEffect to restore results on mount
useEffect(() => {
  const previousResults = getResultsByModuleName('AzureDiscovery'); // Use correct module name
  if (previousResults && previousResults.length > 0) {
    console.log('[DiscoveryHook] Restoring', previousResults.length, 'previous results from store');
...
**Module Names for Each Discovery Type:**
- Azure: `'AzureDiscovery'`
- Applications: `'ApplicationDiscovery'`
- File System: `'FileSystemDiscovery'`
- Google Workspace: `'GoogleWorkspaceDiscovery'`
- Hyper-V: `'HyperVDiscovery'`
- Office 365: `'Office365Discovery'`
- Security Infrastructure: `'SecurityInfrastructureDiscovery'`
- SQL Server: `'SQLServerDiscovery'`
- VMware: `'VMwareDiscovery'`

**Status:**
- ✅ AzureDiscovery - FIXED (useAzureDiscoveryLogic.ts)
- ⚠️ ApplicationDiscovery - PENDING
- ⚠️ FileSystemDiscovery - PENDING
- ⚠️ GoogleWorkspaceDiscovery - PENDING
- ⚠️ HyperVDiscovery - PENDING
- ⚠️ Office365Discovery - PENDING
- ⚠️ SecurityInfrastructureDiscovery - PENDING
- ⚠️ SQLServerDiscovery - PENDING
- ⚠️ VMwareDiscovery - PENDING
...
Copy-Item C:\enterprisediscovery\src\renderer\hooks\use*DiscoveryLogic.ts D:\Scripts\UserMandA\guiv2\src\renderer\hooks\
Copy-Item C:\enterprisediscovery\src\renderer\store\useDiscoveryStore.ts D:\Scripts\UserMandA\guiv2\src\renderer\store\
```
