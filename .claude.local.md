...
**GUIV2 Deployment Build & Run** (REQUIRED for running from deployment directory):

The application MUST be built and run from `C:\enterprisediscovery\` (the deployment directory). The build process requires three separate webpack compilations:

```powershell
# Step 1: Clean old build artifacts
cd C:\enterprisediscovery
powershell -Command "Remove-Item -Recurse -Force .webpack -ErrorAction SilentlyContinue; Remove-Item -Recurse -Force out -ErrorAction SilentlyContinue"

# Step 2: Sync latest files from development directory to deployment
robocopy "D:\Scripts\UserMandA\guiv2" "C:\enterprisediscovery" /MIR /XD node_modules .webpack out .git /XF "*.log" "*.md" "*.ps1" "*.sh" "*.patch" /NFL /NDL /NP

# Step 3: Build all webpack bundles (CRITICAL - must build all three)
cd C:\enterprisediscovery
...
# .webpack/main/main.js (main process)
# .webpack/renderer/main_window/index.html (renderer HTML)
# .webpack/preload/index.js (preload script)
...
- The app references absolute paths to `C:\enterprisediscovery\`
- PowerShell modules are located at `C:\enterprisediscovery\Modules\`
- Data files are at `C:\discoverydata\ljpops\`
- The build must create THREE separate bundles (main, renderer, preload)
- Running `npm run build` only builds the main process - NOT sufficient
- `electron-forge start` fails due to config errors - use manual webpack + npm start instead

**Common Issues**:
- Missing renderer: "ERR_FILE_NOT_FOUND" for index.html → Build renderer bundle
- Missing preload: "Cannot find module preload/index.js" → Build preload bundle
- Old files cached: Clean .webpack directory before rebuilding
