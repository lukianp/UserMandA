...
function main() {
  ...
  if (SINGLE_MODULE) {
    ...
    if (modules.length === 0) {
      console.error(`❌ Module not found: ${SINGLE_MODULE}`);
      process.exit(1);
    }
    console.log(`\n🎯 Generating single module: ${SINGLE_MODULE}`);
  }

  console.log(`\n📊 Found ${modules.length} enabled modules`);
  ...
  console.log('🔨 Generating view components...\n');
  let successCount = 0;
  let errorCount = 0;
  ...
  modules.forEach(module => {
    try {
      ...
      console.error(`❌ Error generating ${module.id}:`, error.message);
      errorCount++;
    }
  });
  ...
  if (!SINGLE_MODULE) {
    console.log('\n🛣️  Generating route configuration...\n');
