...
import { useMailboxMigration } from "../../hooks/useMailboxMigration";
import DataTable from "../../components/DataTable";

export default function MailboxMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useMailboxMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="mailbox-migration-view" data-cy="mailbox-migration-view">
      <h1 className="text-xl font-semibold">Mailbox Migration</h1>
