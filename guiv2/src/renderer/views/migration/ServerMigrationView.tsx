...
import { useServerMigration } from "../../hooks/useServerMigration";
import DataTable from "../../components/DataTable";

export default function ServerMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useServerMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="server-migration-view" data-cy="server-migration-view">
      <h1 className="text-xl font-semibold">Server Migration</h1>
