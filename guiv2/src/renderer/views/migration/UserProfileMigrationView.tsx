...
export default function UserProfileMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useUserProfileMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="user-profile-migration-view" data-cy="user-profile-migration-view">
      <h1 className="text-xl font-semibold">UserProfile Migration</h1>
