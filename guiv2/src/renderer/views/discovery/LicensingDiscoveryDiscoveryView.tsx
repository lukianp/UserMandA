...
import { useLicensingDiscoveryDiscovery } from "../../hooks/useLicensingDiscoveryDiscovery";
import DataTable from "../../components/DataTable";

export default function LicensingDiscoveryDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useLicensingDiscoveryDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="licensing-discovery-discovery-view" data-cy="licensing-discovery-discovery-view">
      <h1 className="text-xl font-semibold">LicensingDiscovery Discovery</h1>
