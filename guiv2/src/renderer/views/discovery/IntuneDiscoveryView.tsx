...
const IntuneDiscoveryView: React.FC = () => {
  ...
  return (
    <div className="min-h-full flex flex-col bg-gray-50 dark:bg-gray-900" data-cy="intune-discovery-view" data-testid="intune-discovery-view">
      {isDiscovering && (
        <LoadingOverlay
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4 p-6">
          ...
          {(() => {
            const expectedSources = 5; // Devices, Apps, Config, Compliance, AppProtection
            const receivedSources = [
              stats.totalDevices > 0,
              stats.totalApplications > 0,
              stats.totalConfigPolicies > 0,
              stats.totalCompliancePolicies > 0,
              stats.totalAppProtectionPolicies > 0
            ].filter(Boolean).length;
            const successPercent = Math.round((receivedSources / expectedSources) * 100);
            const successColor = successPercent >= 80 ? 'from-green-500 to-green-600' :
              successPercent >= 50 ? 'from-yellow-500 to-yellow-600' : 'from-red-500 to-red-600';
            return (
              <div className={`p-4 bg-gradient-to-br ${successColor} rounded-lg shadow text-white`}>
                <div className="flex items-center justify-between">
                  <ShieldCheck className="w-8 h-8 opacity-80" />
                  <div className="text-right">
                    <div className="text-3xl font-bold">{successPercent}%</div>
                    <div className="text-sm opacity-90">Discovery Success</div>
                    <div className="text-xs opacity-75">{receivedSources}/{expectedSources} sources</div>
                  </div>
                </div>
              </div>
            );
