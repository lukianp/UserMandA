...
const WebServerConfigurationDiscoveryView: React.FC = () => {
  const { config, setConfig, result, isDiscovering, progress, filter, setFilter, startDiscovery, cancelDiscovery, columns, filteredData, stats } = useWebServerDiscoveryLogic();
    ...
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900" data-cy="web-server-configuration-discovery-view">
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4 p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="p-4 bg-sky-600/10 rounded-lg">
            <div className="text-2xl font-bold text-sky-600">{stats.totalFound || 0}</div>
      ...
      <div className="flex-1 flex flex-col p-6">
        <div className="flex gap-2 mb-4 border-b border-gray-200 dark:border-gray-700">
          <button onClick={() => setActiveTab(0)} className={`px-4 py-2 ${activeTab === 0 ? 'border-b-2 border-sky-600 text-sky-600' : 'text-gray-600'}`} data-cy="tab-overview">Overview</button>
          <button onClick={() => setActiveTab(1)} className={`px-4 py-2 ${activeTab === 1 ? 'border-b-2 border-sky-600 text-sky-600' : 'text-gray-600'}`} data-cy="tab-servers">Servers</button>
          <button onClick={() => setActiveTab(2)} className={`px-4 py-2 ${activeTab === 2 ? 'border-b-2 border-sky-600 text-sky-600' : 'text-gray-600'}`} data-cy="tab-sites">Sites</button>
          <button onClick={() => setActiveTab(3)} className={`px-4 py-2 ${activeTab === 3 ? 'border-b-2 border-sky-600 text-sky-600' : 'text-gray-600'}`} data-cy="tab-bindings">Bindings</button>
        </div>
