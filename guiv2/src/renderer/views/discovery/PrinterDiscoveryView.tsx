...
export default function PrinterDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = usePrinterDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="printer-discovery-view" data-cy="printer-discovery-view">
      <h1 className="text-xl font-semibold">Printer Discovery</h1>
