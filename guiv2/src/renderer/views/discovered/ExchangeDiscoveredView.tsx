 ...
 *
 * Displays CSV data from ExchangeMailboxes.csv
...
export const ExchangeDiscoveredView: React.FC = () => {
  const componentKeyRef = useRef(`exchange-${Date.now()}`);
  const mountCountRef = useRef(0);
  const [searchText, setSearchText] = useState('');

  useEffect(() => {
    mountCountRef.current += 1;
    console.log(`[Exchange] Component mounted (mount #${mountCountRef.current}, key: ${componentKeyRef.current})`);

    return () => {
      console.log(`[Exchange] Component unmounted (mount #${mountCountRef.current})`);
    };
  }, []);
  ...
  const { data, columns, loading, error, lastRefresh, reload } = useCsvDataLoader(
    'ExchangeMailboxes.csv',
    {
      ...
      onError: (err) => {
        console.error('[Exchange] CSV load error:', err);
      },
      onSuccess: (loadedData, loadedColumns) => {
        console.log(`[Exchange] Data loaded successfully: ${loadedData.length} rows, ${loadedColumns.length} columns`);
      },
    }
  );
  ...
  const handleRefresh = () => {
    console.log('[Exchange] User triggered refresh');
    reload();
  };
  ...
  return (
    <div key={componentKeyRef.current}>
      <DiscoveredViewTemplate
        title="Exchange"
        description="Exchange discovered data from automated scanning"
        data={data}
        columns={columns}
        loading={loading}
        error={error}
