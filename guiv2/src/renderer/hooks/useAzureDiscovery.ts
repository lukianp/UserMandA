...
export function useAzureDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("Azure", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Azure discovery");
    }
  }, [discovery]);
