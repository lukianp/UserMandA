...
export function useIntuneDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("Intune", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Intune discovery");
    }
  }, [discovery]);
