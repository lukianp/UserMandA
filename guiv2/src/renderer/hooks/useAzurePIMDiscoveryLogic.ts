/**
 * Azure PIM Eligible Roles Discovery Hook
 *
 * Event-driven hook for Azure Privileged Identity Management discovery.
...
export interface AzurePIMDiscoveryConfig {
  timeout?: number;
}

export interface AzurePIMDiscoveryProgress {
  percentage: number;
  message: string;
}

export const useAzurePIMDiscoveryLogic = () => {
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any>(null);
  const [progress, setProgress] = useState<AzurePIMDiscoveryProgress>({ percentage: 0, message: '' });
  ...
  const [config] = useState<AzurePIMDiscoveryConfig>({
    timeout: 300000,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeComplete = window.electron?.onDiscoveryComplete?.((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const pimRoles = resultData?.PIMEligibleRoles || resultData?.pimEligibleRoles || [];
        const count = Array.isArray(pimRoles) ? pimRoles.length : 0;
        ...
        addResult({
          id: `azure-pim-${Date.now()}`,
          name: 'Azure PIM Eligible Roles Discovery',
          moduleName: 'AzurePIM',
          displayName: 'PIM Eligible Roles',
          itemCount: count,
          ...
          createdAt: new Date(),
          success: true,
          summary: `Discovered ${count} PIM Eligible Roles`,
          errorMessage: '',
          additionalData: { pimRoles: pimRoles },
        });

        setLogs((prev) => [...prev, { timestamp: new Date().toISOString(), message: `Discovery completed! Found ${count} PIM Eligible Roles.`, level: 'success' }]);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    const token = `azure-pim-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    setIsDiscovering(true);
    setIsCancelling(false);
    setError(null);
    setLogs([{ timestamp: new Date().toISOString(), message: 'Starting PIM Eligible Roles discovery...', level: 'info' }]);
    ...
    try {
      await window.electron?.executeDiscovery?.({
        ...
        parameters: {
          DiscoveryType: 'PIMEligibleRoles',
        },
