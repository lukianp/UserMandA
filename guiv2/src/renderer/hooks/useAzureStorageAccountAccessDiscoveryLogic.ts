/**
 * Azure Storage Account Access Discovery Hook
 *
 * Event-driven hook for Azure Storage Account Access discovery operations.
 ...
 * - Encryption settings and key sources
 * - TLS version and HTTPS-only settings
 */
...
export interface AzureStorageAccountAccessDiscoveryConfig {
  timeout?: number;
}

export interface AzureStorageAccountAccessDiscoveryProgress {
  percentage: number;
  message: string;
}

export const useAzureStorageAccountAccessDiscoveryLogic = () => {
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any>(null);
  const [progress, setProgress] = useState<AzureStorageAccountAccessDiscoveryProgress>({ percentage: 0, message: '' });
  ...
  const [config] = useState<AzureStorageAccountAccessDiscoveryConfig>({
    timeout: 300000,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeComplete = window.electron?.onDiscoveryComplete?.((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const storageAccounts = resultData?.StorageAccountAccess || resultData?.storageAccountAccess || [];
        const count = Array.isArray(storageAccounts) ? storageAccounts.length : 0;
        ...
        addResult({
          id: `azure-storage-access-${Date.now()}`,
          name: 'Azure Storage Account Access Discovery',
          moduleName: 'AzureStorageAccountAccess',
          ...
          createdAt: new Date(),
          success: true,
          summary: `Discovered ${count} Storage Accounts`,
          errorMessage: '',
          additionalData: { storageAccounts: storageAccounts },
        });

        setLogs((prev) => [...prev, { timestamp: new Date().toISOString(), message: `Discovery completed! Found ${count} Storage Accounts.`, level: 'success' }]);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    const token = `azure-storage-access-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    setIsDiscovering(true);
    setIsCancelling(false);
    setError(null);
    setLogs([{ timestamp: new Date().toISOString(), message: 'Starting Storage Account Access discovery...', level: 'info' }]);
    ...
    try {
      await window.electron?.executeDiscovery?.({
        ...
        parameters: {
          DiscoveryType: 'StorageAccountAccess',
        },
