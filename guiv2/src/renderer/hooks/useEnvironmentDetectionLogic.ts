...
import { EnvironmentDetectionConfig, EnvironmentDetectionResult, DetectionFilterState } from '../types/models/environmentdetection';

export const useEnvironmentDetectionLogic = () => {
  const [config, setConfig] = useState<Partial<EnvironmentDetectionConfig>>({ timeout: 300000 });
  const [result, setResult] = useState<EnvironmentDetectionResult | null>(null);
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [progress, setProgress] = useState(0);
  const [filter, setFilter] = useState<DetectionFilterState>({ searchText: '' });
  ...
  const filteredData = useMemo(() => {
    if (!result?.detectedServices) return [];
    return result.detectedServices.filter(item => {
      if (filter.searchText && !JSON.stringify(item).toLowerCase().includes(filter.searchText.toLowerCase())) return false;
      return true;
    });
  }, [result, filter]);

  return { config, setConfig, result, isDiscovering, progress, filter, setFilter, startDiscovery, cancelDiscovery, columns, filteredData, stats: result ? { totalFound: result.totalDetectedservicesFound || 0 } : null };
};
