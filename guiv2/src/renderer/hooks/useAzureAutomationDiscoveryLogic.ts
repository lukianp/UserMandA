/**
 * Azure Automation Accounts Discovery Hook
 *
 * Event-driven hook for Azure Automation Accounts discovery operations.
 * Part of AzureHound-inspired enhancements for comprehensive Azure enumeration.
 *
 * Discovers:
 * - Automation Accounts with runbooks, variables, schedules
 * - Credential counts and configuration
 * - State and modification timestamps
 */
...
export interface AzureAutomationDiscoveryConfig {
  timeout?: number;
}

export interface AzureAutomationDiscoveryProgress {
  percentage: number;
  message: string;
}

export const useAzureAutomationDiscoveryLogic = () => {
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any>(null);
  const [progress, setProgress] = useState<AzureAutomationDiscoveryProgress>({ percentage: 0, message: '' });
  ...
  const [config] = useState<AzureAutomationDiscoveryConfig>({
    timeout: 300000,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeComplete = window.electron?.onDiscoveryComplete?.((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const automationAccounts = resultData?.AutomationAccounts || resultData?.automationAccounts || [];
        const count = Array.isArray(automationAccounts) ? automationAccounts.length : 0;
        ...
        addResult({
          id: `azure-automation-${Date.now()}`,
          name: 'Azure Automation Accounts Discovery',
          moduleName: 'AzureAutomation',
          ...
          createdAt: new Date(),
          success: true,
          summary: `Discovered ${count} Automation Accounts`,
          errorMessage: '',
          additionalData: { automationAccounts: automationAccounts },
        });

        setLogs((prev) => [...prev, { timestamp: new Date().toISOString(), message: `Discovery completed! Found ${count} Automation Accounts.`, level: 'success' }]);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    const token = `azure-automation-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    setIsDiscovering(true);
    setIsCancelling(false);
    setError(null);
    setLogs([{ timestamp: new Date().toISOString(), message: 'Starting Automation Accounts discovery...', level: 'info' }]);
    ...
    try {
      await window.electron?.executeDiscovery?.({
        ...
        parameters: {
          DiscoveryType: 'AutomationAccounts',
        },
