...
export function useAWSDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("AWS", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start AWS discovery");
    }
  }, [discovery]);
