/**
 * Azure Infrastructure Discovery View Logic Hook
 * Handles Azure Infrastructure discovery operations (VMs, Storage, Network, Key Vaults)
 */
...
export interface AzureInfraDiscoveryFormData {
  includeVirtualMachines: boolean;
  includeStorageAccounts: boolean;
  includeNetworkSecurityGroups: boolean;
  includeKeyVaults: boolean;
  timeout: number;
  showWindow: boolean;
}

export const useAzureInfraDiscoveryLogic = () => {
  const selectedSourceProfile = useProfileStore((state) => state.selectedSourceProfile);
  const { addResult, setProgress, getResultsByModuleName } = useDiscoveryStore();

  // Form state
  const [formData, setFormData] = useState<AzureInfraDiscoveryFormData>({
    includeVirtualMachines: true,
    includeStorageAccounts: true,
    includeNetworkSecurityGroups: true,
  ...
  const isFormValid = useMemo(() => {
    ...
    return formData.includeVirtualMachines || formData.includeStorageAccounts ||
           formData.includeNetworkSecurityGroups || formData.includeKeyVaults;
  }, [formData, selectedSourceProfile]);

  // Form handlers
  const updateFormField = useCallback(<K extends keyof AzureInfraDiscoveryFormData>(
    field: K,
    value: AzureInfraDiscoveryFormData[K]
  ) => {
  ...
  useEffect(() => {
    const previousResults = getResultsByModuleName('AzureInfraDiscovery');
    ...
    const unsubscribeComplete = window.electron.onDiscoveryComplete((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const discoveryResult: DiscoveryResult = {
          id: `azure-infra-discovery-${Date.now()}`,
          name: 'Azure Infrastructure Discovery',
          moduleName: 'AzureInfraDiscovery',
          displayName: 'Azure Infrastructure Discovery',
          ...
          success: true,
          summary: `Discovered ${recordCount} infrastructure resources`,
          errorMessage: '',
        ...
        addLog(`Discovery completed! Found ${recordCount} resources.`, 'success');
        setIsRunning(false);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    if (!isFormValid) {
      setError('Please select at least one resource type to discover');
      addLog('Please select at least one resource type', 'error');
      return;
    }
    ...
    const token = `azure-infra-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    addLog(`Starting Infrastructure discovery for ${selectedSourceProfile.companyName}...`, 'info');

    const types: string[] = [];
    if (formData.includeVirtualMachines) types.push('Virtual Machines');
    if (formData.includeStorageAccounts) types.push('Storage Accounts');
    if (formData.includeNetworkSecurityGroups) types.push('NSGs');
    if (formData.includeKeyVaults) types.push('Key Vaults');
    ...
    try {
      await window.electron.executeDiscovery({
        moduleName: 'AzureInfraDiscovery',
        parameters: {
          IncludeVirtualMachines: formData.includeVirtualMachines,
          IncludeStorageAccounts: formData.includeStorageAccounts,
          IncludeNetworkSecurityGroups: formData.includeNetworkSecurityGroups,
          IncludeKeyVaults: formData.includeKeyVaults,
