...
export function useApplicationDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("Application", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Application discovery");
    }
  }, [discovery]);
