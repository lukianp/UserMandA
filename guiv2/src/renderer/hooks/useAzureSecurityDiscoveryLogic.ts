/**
 * Azure Security Discovery View Logic Hook
 * Handles Azure Security discovery operations (CA Policies, Directory Roles, RBAC)
 */
...
export interface AzureSecurityDiscoveryFormData {
  includeConditionalAccess: boolean;
  includeDirectoryRoles: boolean;
  includeRBAC: boolean;
  timeout: number;
  showWindow: boolean;
}

export const useAzureSecurityDiscoveryLogic = () => {
  const selectedSourceProfile = useProfileStore((state) => state.selectedSourceProfile);
  const { addResult, setProgress, getResultsByModuleName } = useDiscoveryStore();

  // Form state
  const [formData, setFormData] = useState<AzureSecurityDiscoveryFormData>({
  ...
  const isFormValid = useMemo(() => {
    ...
    return formData.includeConditionalAccess || formData.includeDirectoryRoles || formData.includeRBAC;
  }, [formData, selectedSourceProfile]);

  // Form handlers
  const updateFormField = useCallback(<K extends keyof AzureSecurityDiscoveryFormData>(
    field: K,
    value: AzureSecurityDiscoveryFormData[K]
  ) => {
  ...
  useEffect(() => {
    const previousResults = getResultsByModuleName('AzureSecurityDiscovery');
    ...
    const unsubscribeComplete = window.electron.onDiscoveryComplete((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const discoveryResult: DiscoveryResult = {
          id: `azure-security-discovery-${Date.now()}`,
          name: 'Azure Security Discovery',
          moduleName: 'AzureSecurityDiscovery',
          displayName: 'Azure Security Discovery',
          ...
          success: true,
          summary: `Discovered ${recordCount} security items`,
          errorMessage: '',
  ...
  const startDiscovery = useCallback(async () => {
    ...
    if (!isFormValid) {
      setError('Please select at least one security area to discover');
      addLog('Please select at least one security area', 'error');
      return;
    }
    ...
    const token = `azure-security-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    addLog(`Starting Azure Security discovery for ${selectedSourceProfile.companyName}...`, 'info');

    const services: string[] = [];
    if (formData.includeConditionalAccess) services.push('Conditional Access');
    if (formData.includeDirectoryRoles) services.push('Directory Roles');
    if (formData.includeRBAC) services.push('RBAC Assignments');
    ...
    try {
      await window.electron.executeDiscovery({
        moduleName: 'AzureSecurityDiscovery',
        parameters: {
          IncludeConditionalAccess: formData.includeConditionalAccess,
          IncludeDirectoryRoles: formData.includeDirectoryRoles,
          IncludeRBAC: formData.includeRBAC,
