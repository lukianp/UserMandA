...
export function useExchangeDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("Exchange", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Exchange discovery");
    }
  }, [discovery]);
