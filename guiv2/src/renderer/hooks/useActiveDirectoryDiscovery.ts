...
export function useActiveDirectoryDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("ActiveDirectory", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Active Directory discovery");
    }
  }, [discovery]);
