...
import { HyperVDiscoveryConfig, HyperVDiscoveryResult, HyperVFilterState } from '../types/models/hyperv';

export const useHyperVDiscoveryLogic = () => {
  const [config, setConfig] = useState<Partial<HyperVDiscoveryConfig>>({ timeout: 300000 });
  const [result, setResult] = useState<HyperVDiscoveryResult | null>(null);
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [progress, setProgress] = useState(0);
  const [filter, setFilter] = useState<HyperVFilterState>({ searchText: '' });
  ...
  const filteredData = useMemo(() => {
    ...
    return result.hosts.filter(item => {
      if (filter.searchText && !JSON.stringify(item).toLowerCase().includes(filter.searchText.toLowerCase())) return false;
      return true;
    });
  }, [result, filter]);

  return { config, setConfig, result, isDiscovering, progress, filter, setFilter, startDiscovery, cancelDiscovery, columns, filteredData, stats: result ? { totalFound: result.totalHostsFound || 0 } : null };
};
