...
export function useUserProfileMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.planMigration({ provider: "UserProfile", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.executeMigration({ provider: "UserProfile", profileId, args });
    setRunId(runId);
  }, [profileId]);
  ...
  useEffect(() => {
    ...
    window.electronAPI.onMigrationComplete(onR);
  }, [runId]);
