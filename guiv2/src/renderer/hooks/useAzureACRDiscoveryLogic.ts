/**
 * Azure Container Registry Discovery Hook
 *
 * Event-driven hook for Azure Container Registry discovery operations.
 * Part of AzureHound-inspired enhancements for comprehensive Azure enumeration.
 *
 * Discovers:
 * - Container Registries with SKU and login server
 * - Admin user and network access settings
 * - Zone redundancy and provisioning state
 */
...
export interface AzureACRDiscoveryConfig {
  timeout?: number;
}

export interface AzureACRDiscoveryProgress {
  percentage: number;
  message: string;
}

export const useAzureACRDiscoveryLogic = () => {
  const [isDiscovering, setIsDiscovering] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any>(null);
  const [progress, setProgress] = useState<AzureACRDiscoveryProgress>({ percentage: 0, message: '' });
  ...
  const [config] = useState<AzureACRDiscoveryConfig>({
    timeout: 300000,
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeComplete = window.electron?.onDiscoveryComplete?.((data) => {
      if (data.executionId === currentTokenRef.current) {
        ...
        const registries = resultData?.ContainerRegistries || resultData?.containerRegistries || [];
        ...
        addResult({
          id: `azure-acr-${Date.now()}`,
          name: 'Azure Container Registry Discovery',
          moduleName: 'AzureACR',
          ...
          createdAt: new Date(),
          success: true,
          summary: `Discovered ${count} Container Registries`,
          errorMessage: '',
          additionalData: { containerRegistries: registries },
        });

        setLogs((prev) => [...prev, { timestamp: new Date().toISOString(), message: `Discovery completed! Found ${count} Container Registries.`, level: 'success' }]);
  ...
  const startDiscovery = useCallback(async () => {
    ...
    const token = `azure-acr-discovery-${Date.now()}`;
    currentTokenRef.current = token;

    setIsDiscovering(true);
    setIsCancelling(false);
    setError(null);
    setLogs([{ timestamp: new Date().toISOString(), message: 'Starting Container Registry discovery...', level: 'info' }]);
    ...
    try {
      await window.electron?.executeDiscovery?.({
        ...
        parameters: {
          DiscoveryType: 'ContainerRegistries',
        },
