...
export function useApplicationDependencyDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("ApplicationDiscovery", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start Application Dependency discovery");
    }
  }, [discovery]);
