...
export function useSharePointMigrationEnhancedMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.planMigration({ provider: "SharePointMigrationEnhanced", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.executeMigration({ provider: "SharePointMigrationEnhanced", profileId, args });
    setRunId(runId);
  }, [profileId]);
