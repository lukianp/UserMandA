...
const SankeyDiagram = ({ nodes, links, width = 1200, height = 600, onNodeClick, onNodeHover, }) => {
    ...
    (0,react.useEffect)(() => {
        ...
        const svg = src/* select */.Ltv(svgRef.current)
            .attr('width', width)
            .attr('height', height)
        ...
        const sankeyGenerator = (0,d3_sankey_src/* sankey */.IZ)()
            .nodeId((d) => d.id)
            .nodeWidth(15)
            .nodePadding(10)
        ...
        if (validLinks.length === 0) {
            ...
            node.append('rect')
                ...
                if (onNodeHover)
                    onNodeHover(d);
                src/* select */.Ltv(this).attr('stroke', '#fbbf24').attr('stroke-width', 3);
            })
                .on('mouseleave', function (event, d) {
                setHoveredNode(null);
                if (onNodeHover)
                    onNodeHover(null);
                src/* select */.Ltv(this).attr('stroke', '#fff').attr('stroke-width', 2);
            })
            ...
            const tooltip = src/* select */.Ltv('body')
                .append('div')
        ...
        const link = g.append('g')
            ...
            .join('path')
            .attr('d', (0,d3_sankey_src/* sankeyLinkHorizontal */.Ku)())
            .attr('stroke', (d) => {
