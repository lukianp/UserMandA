...
if (-not (Get-Command Write-MandALog -ErrorAction SilentlyContinue)) {
    function Write-MandALog {
        ...
        switch ($Level) {
            'ERROR' { Write-Error "[GPODiscovery] $logMessage" }
            'WARN' { Write-Warning "[GPODiscovery] $logMessage" }
            'SUCCESS' { Write-Information "[GPODiscovery] $logMessage" -InformationAction Continue }
            'HEADER' { Write-Verbose "[GPODiscovery] $logMessage" -Verbose }
            'DEBUG' { Write-Verbose "[GPODiscovery] $logMessage" -Verbose }
            default { Write-Information "[GPODiscovery] $logMessage" -InformationAction Continue }
        }
    }
}
