...
if (-not (Get-Command Write-MandALog -ErrorAction SilentlyContinue)) {
    function Write-MandALog {
        ...
        switch ($Level) {
            'ERROR' { Write-Error "[SQLServer] $logMessage" }
            'WARN' { Write-Warning "[SQLServer] $logMessage" }
            'SUCCESS' { Write-Information "[SQLServer] $logMessage" -InformationAction Continue }
            'HEADER' { Write-Verbose "[SQLServer] $logMessage" -Verbose }
            'DEBUG' { Write-Verbose "[SQLServer] $logMessage" -Verbose }
            default { Write-Information "[SQLServer] $logMessage" -InformationAction Continue }
        }
    }
}
...
Export-ModuleMember -Function Invoke-SQLServer
