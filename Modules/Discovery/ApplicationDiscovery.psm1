...
function Get-AzureADApplications {
    ...
    begin {
        Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Starting Azure AD application discovery (Enrichment: $EnrichmentLevel)..." -Level "INFO"
        $discoveredApps = @()
    }
    ...
    process {
        try {
            ...
            if ($IncludeServicePrincipals) {
                ...
                Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Enriching service principal data (this may take a few minutes)..." -Level "INFO"

                $spProcessed = 0
                $spStartTime = Get-Date
                foreach ($sp in $servicePrincipals) {
                    $spProcessed++

                    # Log progress every 25 service principals with ETA
                    if ($spProcessed % 25 -eq 0 -or $spProcessed -eq $servicePrincipals.Count) {
                        $elapsed = (Get-Date) - $spStartTime
                        $avgTimePerSP = $elapsed.TotalSeconds / $spProcessed
                        $remaining = $servicePrincipals.Count - $spProcessed
                        $etaSeconds = [math]::Round($remaining * $avgTimePerSP)
                        $etaMin = [math]::Floor($etaSeconds / 60)
                        $etaSec = $etaSeconds % 60
                        Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Processing service principal $spProcessed of $($servicePrincipals.Count) (ETA: ${etaMin}m ${etaSec}s)..." -Level "INFO"
                    }
                    ...
                    # Enrichment based on level
                    if ($EnrichmentLevel -ne 'None') {
                        # Basic enrichment: Only get owners (reduces 984 API calls to 246)
                        try {
                            ...
                            # Silently continue on owner fetch failure
                        }

                        # Full enrichment: Get permissions, grants, and sign-in activity
                        if ($EnrichmentLevel -eq 'Full') {
                            # Get app role assignments TO this service principal
                            try {
                            ...
                            # Get app role assignments FROM this service principal
                            try {
                            ...
                            try {
                                ...
                                # Sign-in activity might not be available
                            }
                        }
                    }
                    ...
                    $appData = [PSCustomObject]@{
                        ...
                        Vendor = if ($sp.PSObject.Properties['publisherName'] -and $sp.publisherName) { $sp.publisherName } else { "Unknown" }
            ...
            if ($IncludeAppRegistrations) {
                ...
                Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Enriching app registration data (this may take a few minutes)..." -Level "INFO"
                ...
                foreach ($app in $appRegistrations) {
                    ...
                    # Log progress every 25 app registrations with ETA
                    if ($appProcessed % 25 -eq 0 -or $appProcessed -eq $appRegistrations.Count) {
                        $elapsed = (Get-Date) - $appStartTime
                        $avgTimePerApp = $elapsed.TotalSeconds / $appProcessed
                        $remaining = $appRegistrations.Count - $appProcessed
                        $etaSeconds = [math]::Round($remaining * $avgTimePerApp)
                        $etaMin = [math]::Floor($etaSeconds / 60)
                        $etaSec = $etaSeconds % 60
                        Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Processing app registration $appProcessed of $($appRegistrations.Count) (ETA: ${etaMin}m ${etaSec}s)..." -Level "INFO"
                    }
                    ...
                    # Enrichment based on level
                    if ($EnrichmentLevel -ne 'None') {
                        # Basic enrichment: Only get owners
                        try {
                        ...
                        # Full enrichment: Get federated credentials and detailed permissions
                        if ($EnrichmentLevel -eq 'Full') {
                    ...
                    $appData = [PSCustomObject]@{
                        ...
                        Vendor = if ($app.publisherDomain) { $app.publisherDomain } else { "Unknown" }
...
function Export-ApplicationCatalog {
    ...
    try {
        ...
        if ($Format -in @('CSV', 'All')) {
            $csvPath = Join-Path $OutputPath "ApplicationCatalog.csv"
        ...
        if ($Format -in @('JSON', 'All')) {
            $jsonPath = Join-Path $OutputPath "ApplicationCatalog.json"
            $catalogData = @{
...
function Invoke-ApplicationDiscovery {
    ...
    try {
        ...
        if (@($allDiscoveredData).Count -gt 0) {
            ...
            $catalogResult = New-ApplicationCatalog -EnrichedApps $allDiscoveredData
            ...
            if ($Configuration.ContainsKey('TestConnectivity') -and $Configuration.TestConnectivity -and @($applicationCatalog).Count -gt 0) {
                Write-ModuleLog -ModuleName "ApplicationDiscovery" -Message "Testing application connectivity for top applications..." -Level "INFO"
                $topApplications = $applicationCatalog | Where-Object { $_.URL -and $_.URL -like "https://*" } | Select-Object -First 10
            ...
            $result.RecordCount = @($applicationCatalog).Count
        } else {
