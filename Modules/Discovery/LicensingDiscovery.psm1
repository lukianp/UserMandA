...
if (-not (Get-Command Write-MandALog -ErrorAction SilentlyContinue)) {
    function Write-MandALog {
        ...
        switch ($Level) {
            'ERROR' { Write-Error "[LicensingDiscovery] $logMessage" }
            'WARN' { Write-Warning "[LicensingDiscovery] $logMessage" }
            'SUCCESS' { Write-Information "[LicensingDiscovery] $logMessage" -InformationAction Continue }
            'HEADER' { Write-Verbose "[LicensingDiscovery] $logMessage" -Verbose }
            'DEBUG' { Write-Verbose "[LicensingDiscovery] $logMessage" -Verbose }
            default { Write-Information "[LicensingDiscovery] $logMessage" -InformationAction Continue }
        }
    }
}
