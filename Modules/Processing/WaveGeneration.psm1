    ...
    try {
        ...
        function New-WaveObjectLocal { # Renamed to avoid conflict if New-WaveObject is global
            ...
            return [PSCustomObject]@{
                ...
                ClusterId          = $ClusterId
                Notes              = $Notes
                UserProfilesInWave = $WaveUserProfiles
            }
        }
        ...
        Write-MandALog "Error in function 'New-MigrationWaves': $($_.Exception.Message)" "ERROR"
        throw
    }
}
...
function Test-WaveDependencies {
    param(
        [Parameter(Mandatory = $true)]
        [System.Collections.IList]$Waves,
        [Parameter(Mandatory = $true)]
        [System.Collections.IList]$Profiles
    )
    ...
    try {
            ...
            if ($userProfile.PSObject.Properties['RequiresCoMigration'] -and $userProfile.RequiresCoMigration -and $userProfile.PSObject.Properties['CoMigrationUsers'] -and $userProfile.CoMigrationUsers) {
                ...
                foreach ($dependentUserUPN in $userProfile.CoMigrationUsers) {
                    ...
                    if ($null -ne $dependentUserWaveOrder) {
                        ...
                        $LogFn.Invoke("Co-dependent user '$dependentUserUPN' for '$currentUserUPN' not found in any wave during validation.", "DEBUG")
                    }
                }
            }
        }
        ...
        if ($issues.Count -gt 0) {
            ...
            $LogFn.Invoke("No co-migration dependency issues found in wave planning.", "SUCCESS")
        }
        
        return $issues
    } catch {
        Write-MandALog "Error in function 'Test-WaveDependencies': $($_.Exception.Message)" "ERROR"
        throw
    }
}
...
function Optimize-SharedMailboxWaves {
    ...
    try {
        ...
        foreach ($wave in $Waves) {
            $waveUPNs = $wave.UserPrincipalNames -split ';' | Where-Object { $_.Trim() }
            foreach ($upn in $waveUPNs) { $userToWaveIDLookup[$upn.Trim()] = $wave.WaveID }
        }
            ...
            foreach ($sharedMbxAccessInfo in $userProfile.ExchangePermissions.SharedMailboxAccess) {
                ...
                if (-not $sharedMailboxStats.ContainsKey($mbxEmail)) {
                    $sharedMailboxStats[$mbxEmail] = @{
                        ...
                        Users = [System.Collections.Generic.List[string]]::new()
                        WaveIDs = [System.Collections.Generic.List[string]]::new()
                        UserWaveMapping = @{}
                    }
                }
        ...
        return @{
            SharedMailboxAccessDetails = $sharedMailboxStats
            OptimizationSuggestions = $optimizationCandidates
        }
    } catch {
        Write-MandALog "Error in function 'Optimize-SharedMailboxWaves': $($_.Exception.Message)" "ERROR"
        throw
    }
}
