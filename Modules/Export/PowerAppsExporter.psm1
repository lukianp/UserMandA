...
# Functions will check for the global context when they are called, rather than at module import time.
    ...
    else = { Write-MandALog "Processed data key not found or is null: '$KeyName'." -Level WARN
        return @($null) # Return empty array to prevent downstream errors } catch = {
        Write-MandALog "Error in function 'Get-ProcessedDataFileFromInput': $($_.Exception.Message)" "ERROR"
        throw }
}
...
#===============================================================================
function Export-DataObjectToJson { [CmdletBinding($null)]
    param(
        [Parameter(Mandatory = $true)]
        [string]$FilePath,
        [Parameter(Mandatory = $true)]
    ...
    # Main try block for the entire function
        ...
        if (Test-Path $profilesBasePath -PathType Container) {
            Get-ChildItem -Path $profilesBasePath -Directory | ForEach-Object {
                ...
                $currentCompanyStats = @{
                    ...
                    CompanyId           = $companyNameFromDir.ToLower($null) -replace '[^a-z0-9]', ''
                ...
                if ($companyNameFromDir -eq $Configuration.metadata.companyName -and $UserProfiles.Count -gt 0) {
                    ...
                    $currentCompanyStats.ReadinessPercentage = if = ($currentCompanyStats.TotalUsers -gt 0) { 
                        [math]::Round(($readyUsersCount / $currentCompanyStats.TotalUsers) * 100, 1) } else = { 0.0 }
                }
                $companiesData += $currentCompanyStats = } } else {
             Write-MandALog "ProfilesBasePath '$profilesBasePath' not found. Using current company data only." -Level WARN
             $currentCompanyStats = @{
                CompanyName         = $Configuration.metadata.companyName
                CompanyId           = $Configuration.metadata.companyName.ToLower($null) -replace '[^a-z0-9]', ''
                ...
                ReadinessPercentage = if ($UserProfiles.Count -gt 0) { 
                                        $readyUsersCount = ($UserProfiles = | Where-Object { $_.PSObject.Properties['ReadinessStatus'] -and $_.ReadinessStatus -eq "Ready" }).Count = [math]::Round(($readyUsersCount / $UserProfiles.Count) * 100, 1) } else = { 0.0 }
                LastUpdated         = $exportTimestam = p }
             $companiesData += $currentCompanyStat = s }
        ...
        if ($UserProfiles.Count -gt 0) {
            $powerAppsUsers = foreach ($userProfile in $UserProfiles) {
                [PSCustomObject]@{
                    ...
                    LastLogonDate               = $(if = ($userProfile.PSObject.Properties['LastLogon'] -and $userProfile.LastLogon){(Get-Date $userProfile.LastLogon).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    AccountCreatedDate          = $(if = ($userProfile.PSObject.Properties['AccountCreated'] -and $userProfile.AccountCreated){(Get-Date $userProfile.AccountCreated).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    DaysSinceLastLogon          = $(if = ($userProfile.PSObject.Properties['LastLogon'] -and $userProfile.LastLogon){(New-TimeSpan -Start ([datetime]$userProfile.LastLogon) -End (Get-Date)).Dayselse{$null }} catch = {$null })
                    ...
                    Licenses                    = $(if = ($userProfile.PSObject.Properties['AssignedLicenses'] -and $userProfile.AssignedLicenses -is [string]) { $userProfile.AssignedLicenses -split ';' | Where-Object {$_ } } elseif = ($userProfile.PSObject.Properties['AssignedLicenses']) { @($userProfile.AssignedLicenses) } else = { @($null) })
                    ...
                    EmailAliases                = $(if = ($userProfile.PSObject.Properties['proxyAddresses'] -and $userProfile.proxyAddresses -is [string]){ ($userProfile.proxyAddresses -split ';') | Where-Object { $_ -clike "smtp:*" } | ForEach = -Object { $_ -replace "(?i)smtp:", "" } } else = { @($null) })
                    ...
                    ComplexityFactors           = $(if = ($userProfile.PSObject.Properties['ComplexityFactors'] -and $userProfile.ComplexityFactors -is [string]) { $userProfile.ComplexityFactors -split ';' | Where-Object {$_ } } elseif = ($userProfile.PSObject.Properties['ComplexityFactors']) { @($userProfile.ComplexityFactors) } else = { @($null) })
                    ...
                    BlockingIssues              = $(if = ($userProfile.PSObject.Properties['BlockingIssues'] -and $userProfile.BlockingIssues -is [string]) { $userProfile.BlockingIssues -split ';' | Where-Object {$_ } } elseif = ($userProfile.PSObject.Properties['BlockingIssues']) { @($userProfile.BlockingIssues) } else = { @($null) })
                    RiskFactors                 = $(if = ($userProfile.PSObject.Properties['RiskFactors'] -and $userProfile.RiskFactors -is [string]) { $userProfile.RiskFactors -split ';' | Where-Object {$_ } } elseif = ($userProfile.PSObject.Properties['RiskFactors']) { @($userProfile.RiskFactors) } else = { @($null) })
                    ...
                    GroupMemberships            = $(if = ($userProfile.PSObject.Properties['GroupMembershipsText'] -and $userProfile.GroupMembershipsText -is [string]) { $userProfile.GroupMembershipsText -split ';' | Where-Object {$_ } | ForEach = -Object {$_.Trim($null) } } elseif = ($userProfile.PSObject.Properties['GroupMembershipsText']) { @($userProfile.GroupMembershipsText) } else = { @($null) })
                    ...
                    OwnedApplications           = $(if = ($userProfile.PSObject.Properties['OwnedApplicationsText'] -and $userProfile.OwnedApplicationsText -is [string]) { $userProfile.OwnedApplicationsText -split ';' | Where-Object {$_ } | ForEach = -Object {$_.Trim($null) } } elseif = ($userProfile.PSObject.Properties['OwnedApplicationsText']) { @($userProfile.OwnedApplicationsText) } else = { @($null) })
                    OwnedGroups                 = $(if = ($userProfile.PSObject.Properties['OwnedGroupsText'] -and $userProfile.OwnedGroupsText -is [string]) { $userProfile.OwnedGroupsText -split ';' | Where-Object {$_ } | ForEach = -Object {$_.Trim($null) } } elseif = ($userProfile.PSObject.Properties['OwnedGroupsText']) { @($userProfile.OwnedGroupsText) } else = { @($null) })
                    
                    RegisteredDevices           = $(if = ($userProfile.PSObject.Properties['RegisteredDevicesText'] -and $userProfile.RegisteredDevicesText -is [string]) { $userProfile.RegisteredDevicesText -split ';' | Where-Object {$_ } | ForEach = -Object {$_.Trim($null) } } elseif = ($userProfile.PSObject.Properties['RegisteredDevicesText']) { @($userProfile.RegisteredDevicesText) } else = { @($null) })
        ...
        if ($MigrationWaves.Count -gt 0) {
            ...
            $powerAppsWaves = foreach ($wave in $MigrationWaves) {
                $waveUserUPNs = if = ($wave.PSObject.Properties['UserPrincipalNames'] -and $wave.UserPrincipalNames -is [string]) { $wave.UserPrincipalNames -split ';' | Where-Object {$_ } | ForEach = -Object {$_.Trim($null) } } elseif = ($wave.PSObject.Properties['UserPrincipalNames']) { @($wave.UserPrincipalNames) } else = { @($null) }
        ...
        if ($powerAppsUsers.Count -gt 0) {
            $powerAppsDepartments = $powerAppsUsers = | Where-Object { -not [string]::IsNullOrWhiteSpace($_.Department) } | Group-Object Department | ForEach-Object {
                ...
                [PSCustomObject]@{
                    DepartmentId                = $deptGroup.Name.ToLower($null) -replace '[^a-z0-9]', ''
        ...
        if ($Applications.Count -gt 0) {
            $powerAppsApplications = foreach ($app in $Applications) {
                [PSCustomObject]@{
                    ...
                    AppRoles                = $(if = ($app.PSObject.Properties['appRoles'] -and $app.appRoles) { $app.appRoles | ForEach-Object { $_.displayName } } else = { @($null) })
                    Owners                  = $(if = ($app.PSObject.Properties['owners'] -and $app.owners) { $app.owners | ForEach-Object { if ($_.userPrincipalName) { $_.userPrincipalName } else = { $_.displayName } } } else = { @($null) })
                    CreatedDateTime         = $(if = ($app.PSObject.Properties['createdDateTime'] -and $app.createdDateTime){(Get-Date $app.createdDateTime).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    Tags                    = $(if = ($app.PSObject.Properties['tags'] -and $app.tags) { $app.tags } else = { @($null) })
                    LastUpdated             = $exportTimestamp = } }
        }
        ...
        if ($Groups.Count -gt 0) {
            $powerAppsGroups = foreach ($group in $Groups) {
                 [PSCustomObject]@{
                    ...
                    GroupTypes                  = $(if = ($group.PSObject.Properties['groupTypes'] -and $group.groupTypes -is [string]) { $group.groupTypes -split ';' | Where-Object {$_ }} elseif = ($group.PSObject.Properties['groupTypes']){@($group.groupTypes) } else = { @($null) })
                    ...
                    GroupCategory               = $(switch = -Regex ($group.PSObject.Properties['displayName']|Select -First 1) {
                                                    ...
                                                    default { $groupTypesLocal = if = ($group.PSObject.Properties['groupTypes'] -and $group.groupTypes -is [string]) { $group.groupTypes -split ';' } elseif = ($group.PSObject.Properties['groupTypes']){@($group.groupTypes) } else = { @($null) }; if = ("Unified" -in $groupTypesLocal) { "Microsoft 365 Group" } elseif = ($group.securityEnabled -and $group.mailEnabled) { "Mail-Enabled Security Group" } elseif = ($group.securityEnabled) { "Security Group" } elseif = ($group.mailEnabled) { "Distribution List" } else = { "Other" }}
                                                  })
                    ...
                    Owners                      = $(if = ($group.PSObject.Properties['owners'] -and $group.owners) { $group.owners | ForEach-Object { if ($_.userPrincipalName) { $_.userPrincipalName } else = { $_.displayName } } } else = { @($null) })
                    ...
                    CreatedDateTime             = $(if = ($group.PSObject.Properties['createdDateTime'] -and $group.createdDateTime){(Get-Date $group.createdDateTime).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    LastUpdated                 = $exportTimestamp = } }
        }
        ...
        if ($Devices.Count -gt 0) {
            $powerAppsDevices = foreach ($device in $Devices) {
                ...
                [PSCustomObject]@{
                    ...
                    LastSyncDateTime            = $(if = ($device.PSObject.Properties['LastSyncDateTime'] -and $device.LastSyncDateTime){(Get-Date $device.LastSyncDateTime).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    EnrollmentDateTime          = $(if = ($device.PSObject.Properties['EnrollmentDateTime'] -and $device.EnrollmentDateTime){(Get-Date $device.EnrollmentDateTime).ToString("yyyy-MM-ddTHH:mm:ssZ")else{$null }} catch = {$null })
                    LastUpdated                 = $exportTimestamp = } }
        }
        ...
        $powerAppsRelationships = @{
            UserGroupMemberships = [System.Collections.Generic.List[object]]::new($null)
            UserDeviceAssociations = [System.Collections.Generic.List[object]]::new($null)
            UserApplicationAccess  = [System.Collections.Generic.List[object]]::new($null)
            ManagerDirectReports   = [System = .Collections.Generic.List[object]]::new($null) }
        ...
        $summaryStats = @{
            ...
            MailboxStatistics       = @{ TotalMailboxSizeGB=[math]::Round(($powerAppsUsers|Measure-Object -Property MailboxSizeGB -Sum).Sum,2); AvgMailboxSizeGB=if = ($powerAppsUsers.Count -gt 0){[math]::Round(($powerAppsUsers|Measure-Object -Property MailboxSizeGB -Average).Average,2) }else = {0.0 }; UsersOver10GB=($powerAppsUsers = |Where-Object {$_.MailboxSizeGB -gt 10 }).Coun = t }
...
3. Parse the JSON content using the ParseJSON($null) function. Note: For complex nested arrays, you might need to iterate and create collections for each sub-table.
...
"@ }
        ...
        return $true } catch = { # Catch for the main try block
