    ...
    try {
            ...
            Get-ChildItem -Path $profilesBasePath -Directory | ForEach-Object {
             ...
             $currentCompanyStats = @{
                ...
                ReadinessPercentage = if ($UserProfiles.Count -gt 0) { 
                                        $readyUsersCount = ($UserProfiles | Where-Object { $_.PSObject.Properties['ReadinessStatus'] -and $_.ReadinessStatus -eq "Ready" }).Count
                                        [math]::Round(($readyUsersCount / $UserProfiles.Count) * 100, 1) } else { 0.0 }
        ...
        if ($UserProfiles.Count -gt 0) {
            $powerAppsUsers = foreach ($userProfile in $UserProfiles) {
                [PSCustomObject]@{
                    ...
                    SearchTags                  = $(@($userProfile.UserPrincipalName, $userProfile.DisplayName, ($userProfile.PSObject.Properties['Department']|Select-Object -First 1), ($userProfile.PSObject.Properties['Title']|Select-Object -First 1), ($userProfile.PSObject.Properties['MigrationCategory']|Select-Object -First 1), ($userProfile.PSObject.Properties['ReadinessStatus']|Select-Object -First 1)) | Where-Object { $_ } | ForEach-Object { $_.ToLower().Trim() })
        ...
        if ($Groups.Count -gt 0) {
            $powerAppsGroups = foreach ($group in $Groups) {
                 [PSCustomObject]@{
                    ...
                    GroupCategory               = $(switch -Regex ($group.PSObject.Properties['displayName']|Select-Object -First 1) {
                    ...
                    MemberCount                 = ($powerAppsUsers | Where-Object {$_.GroupMemberships -contains $group.displayName -or $_.GroupMemberships -contains ($group.PSObject.Properties['mail']|Select-Object -First 1) -or $_.GroupMemberships -contains ($group.PSObject.Properties['id']|Select-Object -First 1) }).Count
                    ...
                    SourceSystem                = $(if (($group.PSObject.Properties['Id']|Select-Object -First 1) -like "*-*-*-*-*") { "AzureAD" } elseif ($group.PSObject.Properties['ObjectGUID']) { "ActiveDirectory" } else { "Unknown" })
        ...
        if ($Devices.Count -gt 0) {
            $powerAppsDevices = foreach ($device in $Devices) {
                $ownerUPN = ($powerAppsUsers | Where-Object {$_.RegisteredDevices -contains ($device.PSObject.Properties['DeviceId']|Select-Object -First 1) -or $_.RegisteredDevices -contains ($device.PSObject.Properties['DisplayName']|Select-Object -First 1) } | Select-Object -First 1).UPN
                [PSCustomObject]@{
