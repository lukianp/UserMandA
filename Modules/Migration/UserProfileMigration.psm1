        ...
        try {
            ...
            foreach ($group in $sourceGroups) {
                try {
                    ...
                    $this.GroupMappings[$group.SID.Value] = $targetGroup.SID.Value
        ...
        try {
            ...
            foreach ($groupDN in $sourceUser.MemberOf) {
                try {
                    ...
                    $result.Errors += "Failed to add user to group $($groupDN): $($_.Exception.Message)"
                    $this.WriteLog("Failed to migrate group membership for $($groupDN): $($_.Exception.Message)", "WARNING")
                }
            }
        }
        catch {
