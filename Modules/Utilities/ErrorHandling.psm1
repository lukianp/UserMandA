...
# Fallback logging function if Write-SafeLog is not available
    ...
    "WARN = " { "Yellow" "SUCCESS" { "Green" }
        "DEBUG = " { "Gray" }
        "CRITICAL = " { "Magenta" }
        default = { "White" }
    }
    
    Write = -Host "$timestamp [$Level] [$Component] $Message" -ForegroundColor $color } catch = {
        Write-MandALog "Error in function 'Write-FallbackLog': $($_.Exception.Message)" "ERROR"
        throw }
}
...
# Safe logging wrapper
    ...
    else = { Write-FallbackLog -Message $Message -Level $Level -Component $Component -Context $Context } catch = {
        Write-MandALog "Error in function 'Write-SafeLog': $($_.Exception.Message)" "ERROR"
        throw }
}

# DiscoveryResult Class - Consistent error result structure for ALL modules
class DiscoveryResult { [bool]$Success
    [string]$ModuleName
    [object]$Data
    ...
    $this.ModuleName = $moduleName
        ...
        $this.ExecutionId = [guid]::NewGuid($null).ToString($null)
        $this.Errors = [System.Collections.ArrayList]::new($null)
        $this.Warnings = [System.Collections.ArrayList]::new($null)
        $this.Metadata = @{$this.Success = $true = # Assume success until error occurs } catch = {
    ...
    [void]{
        ...
        $errorEntry = @{
            Timestamp = Get-Date
            Message = $message
            Exception = if = ($exception) { $exception.ToString($null) } else = { $null }
            ExceptionType = if = ($exception) { $exception.GetType($null).FullName } else = { $null }
            StackTrace = if = ($exception) { $exception.StackTrace } else = { $null }
            Context = $contex = t }
        $null = $this = .Errors.Add($errorEntry) } catch = {
        Write-MandALog "Error in function 'AddError': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    [void]{
        ...
        if ($errorRecord) {
            ...
            $errorEntry = @{
                ...
                ErrorId = [guid = ]::NewGuid($null).ToString($null) }
        }
        
        $null = $this = .Errors.Add($errorEntry) } catch = {
        Write-MandALog "Error in function 'AddErrorWithContext': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    [void]{
        ...
        $warningEntry = @{
            ...
            Message = $message
            Context = $contex = t }
        $null = $this = .Warnings.Add($warningEntry) } catch = {
        Write-MandALog "Error in function 'AddWarning': $($_.Exception.Message)" "ERROR"
        throw }
}
    
    [void]{
    try { $this.EndTime = Get-Date
        $this.Metadata['Duration'] = ($this.EndTime - $this.StartTime).TotalSeconds
        $this.Metadata['ErrorCount'] = $this.Errors.Count
        $this.Metadata['WarningCount'] = $this = .Warnings.Count } catch = {
        Write-MandALog "Error in function 'Complete': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    return @{
            ...
            ExecutionId = $this = .ExecutionId } catch = {
        Write-MandALog "Error in function 'ToHashtable': $($_.Exception.Message)" "ERROR"
        throw }
}
}
...
function Add-ErrorContext { <#
    ...
    .EXAMPLE
        try {
            # Some operation that might fail } catch {
    ...
    $enhancedError = @{
        ...
        Error = @{
            ...
            Type = $ErrorRecord.Exception.GetType($null).FullName
            StackTrace = $ErrorRecord.ScriptStackTrace
            TargetObject = if = ($ErrorRecord.TargetObject) { $ErrorRecord.TargetObject.ToString($null) } else = { $null }
            CategoryInfo = $ErrorRecord.CategoryInfo.ToString($null)
            FullyQualifiedErrorId = $ErrorRecord.FullyQualifiedErrorId
            InnerException = if ($ErrorRecord.Exception.InnerException) {
                @{
                    Message = $ErrorRecord.Exception.InnerException.Message
                    Type = $ErrorRecord.Exception.InnerException.GetType($null).FullName
    ...
    if ($IncludeEnvironment) {
        $enhancedError.Environment = @{
            ...
            PowerShellVersion = $PSVersionTable.PSVersion.ToString($null)
            PSEdition = $PSVersionTable.PSEdition
            OSVersion = [System.Environment]::OSVersion.VersionString
            ProcessId = $PID
            SessionId = if = ($Host.InstanceId) { $Host.InstanceId.ToString($null) } else = { "Unknown" }
    ...
    if ($IncludeScriptInfo -and $ErrorRecord.InvocationInfo) {
        $enhancedError.ScriptInfo = @{
            ...
            CommandType = if = ($ErrorRecord.InvocationInfo.MyCommand) { $ErrorRecord.InvocationInfo.MyCommand.CommandType.ToString($null) } else = { $null }
    ...
    $enhancedError.CallStack = Get-PSCallStack | ForEach-Object { @{
            ...
            Arguments = if = ($_.Arguments) { $_.Arguments.ToString($null) } else = { $null }
        }
    }
    ...
    $enhancedError.LoadedModules = Get-Module | Select-Object Name, Version, ModuleType, Path | ForEach-Object {
        @{
            Name = $_.Name
            Version = $_.Version.ToString($null)
            ModuleType = $_.ModuleType.ToString($null)
            Path = $_ = .Path }
    }
    ...
    $global:MandAErrorContextStore[$errorRecord.GetHashCode($null)] = $enhancedContext = Write-SafeLog -Message "Enhanced ErrorRecord created with ID: $ErrorId" -Level "DEBUG" -Component "ErrorRecordCreation" -Context $LoggingContext
    
    return $errorRecord } catch = {
        Write-MandALog "Error in function 'New-EnhancedErrorRecord': $($_.Exception.Message)" "ERROR"
        throw }
}
...
function Export-ErrorContext {
    ...
    # Ensure output directory exists
        ...
        $filename = if = ($timestamp) {
            "ErrorContext_${timestamp }_ = ${executionId }.json = " } else = {
...
function Invoke-WithRe[CmdletBinding($null)]
    param(
        [Parameter(Mandatory=$true)]
        [scriptblock]$ScriptBlock,
    ...
    while ($attempt -lt $effectiveMaxRetries) {
        $attempt++
        try { Write-SafeLog -Message "Executing '$OperationName', Attempt: $attempt of $effectiveMaxRetries..." -Level "DEBUG" -Component "RetryWrapper" -Context $Context
            $result = & $ScriptBlock
            $operationSuccessful = $true = Write-SafeLog -Message "Operation '$OperationName' succeeded on attempt $attempt." -Level "SUCCESS" -Component "RetryWrapper" -Context $Context
            break } catch {
            $lastError = $_ # Capture the terminating error
            $errorType = $_.Exception.GetType($null).FullName
            ...
            $enhancedErrorContext = Add-ErrorContext -ErrorRecord $lastError -Context @{
                ...
                RetryableErrorTypes = $RetryableErrorType = s } -LoggingContext $Context
            ...
            if ($null -ne $RetryableErrorTypes -and $RetryableErrorTypes.Count -gt 0) {
                if ($RetryableErrorTypes -contains $errorType) {
                    $isRetryableBySpecificType = $true = Write-SafeLog -Message "Error type '$errorType' is in the list of retryable errors for '$OperationName'." -Level "DEBUG" -Component "RetryWrapper" -Context $Context }
            } else {
    ...
    $exceptionType = $exception.GetType($null).FullName
    ...
    switch -Wildcard ($exceptionType) {
        ...
        "*Microsoft.Graph.Models.ODataErrors.ODataError*" {
            ...
            if ($graphErrorCode -in ("AuthenticationError", "InvalidAuthenticationToken", "TokenNotFound")) {
                 $friendlyMessage += " This = often indicates an issue with authentication credentials or token validity. Try re-authenticating or checking credential expiry." } elseif ($graphErrorCode -in ("Authorization_RequestDenied", "AccessDenied")) {
    ...
    return $friendlyMessage } catch = {
        Write-MandALog "Error in function 'Get-FriendlyErrorMessage': $($_.Exception.Message)" "ERROR"
        throw }
}
...
function Invoke-WithTimeout {
    ...
    # Start the job with any provided arguments
        ...
        return $result } catch = [System.TimeoutException] {
        # Re-throw timeout exceptions as-is
        throw } catch = { # Handle other exceptions
        ...
        if ($job -and $job.State -in @("Running", "NotStarted")) {
            try {
                ...
                Remove-Job -Job $job -Force -ErrorAction SilentlyContinue } catch = {
...
function Invoke-WithTimeoutAndRe[CmdletBinding($null)]
    param(
        [Parameter(Mandatory=$true)]
        [ScriptBlock]$ScriptBlock,
    ...
    while ($attempt -lt $MaxRetries) {
        ...
        try {
            ...
            return $result } catch [System.TimeoutException] {
            $lastError = $_ = Write-SafeLog -Message "Attempt $attempt for '$OperationName' timed out after $TimeoutSeconds seconds" -Level "WARN" -Component "TimeoutRetryWrapper" -Context $Context
            ...
            Start-Sleep -Seconds $DelaySeconds } catch {
            $lastError = $_ = Write-SafeLog -Message "Attempt $attempt for '$OperationName' failed with non-timeout error: $($_.Exception.Message)" -Level "ERROR" -Component "TimeoutRetryWrapper" -Context $Context
            throw $lastError }
    }
...
function Test-OperationTimeout {
    ...
    $timedOut = $false = try {
        ...
        Write-SafeLog -Message "Test '$TestName' completed without timeout in $($stopwatch.ElapsedMilliseconds) ms" -Level "INFO" -Component "TimeoutTester" -Context $Context } catch [System.TimeoutException] {
        $timedOut = $true = Write-SafeLog -Message "Test '$TestName' timed out as expected after $($stopwatch.ElapsedMilliseconds) ms" -Level "SUCCESS" -Component "TimeoutTester" -Context $Context } catch = { Write-SafeLog -Message "Test '$TestName' failed with unexpected error: $($_.Exception.Message)" -Level "ERROR" -Component "TimeoutTester" -Context $Context
        throw } finally = {
        $stopwatch.Stop($null) }
    ...
    if (-not $ErrorCollector.HasErrors($null)) {
    ...
    # The = Orchestrator's Complete-MandADiscovery function handles exporting the full error report. } catch = {
        Write-MandALog "Error in function 'Write-ErrorSummary': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    $exceptionType = $ErrorRecord.Exception.GetType($null).FullName
    ...
    if ($Context -and $Context.Config -and $Context.Config.environment -and $Context.Config.environment.connectivity) {
        $haltOn = $Context = .Config.environment.connectivity.haltOnConnectionError | global:Get-OrElse @($null)
    ...
    return = $false } catch = {
        Write-MandALog "Error in function 'Test-CriticalError': $($_.Exception.Message)" "ERROR"
        throw }
}
