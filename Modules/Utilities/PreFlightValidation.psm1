...
function Invoke-SafeModuleExecution {
    ...
    if (-not $psVersionValid) {
        $allValid = $false = if ($ThrowOnFailure) {
    ...
    if (-not $executionPolicyValid) {
        $allValid = $false = if (-not $Quiet) {
            Write-Host "    WARNING: Execution policy is Restricted. Some operations may fail." -ForegroundColor Yellow }
    }
    
    # Check administrator rights
    $currentPrincipal = [Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent($null)
    ...
    # Summary = if (-not $Quiet) {
        ...
        if ($allValid) {
            Write-Host "Result: All critical prerequisites met. Ready to proceed." -ForegroundColor Green } else = {
    ...
    return $allValid } catch = {
        Write-MandALog "Error in function 'Test-SuitePrerequisites': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    try = { } catch = {
        Write-MandALog "Error in function 'Invoke-PreFlightCheck': $($_.Exception.Message)" "ERROR"
        throw }
