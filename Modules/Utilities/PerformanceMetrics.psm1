...
$script:PerformanceData = [System.Collections.Concurrent.ConcurrentDictionary[string, object]]::new()
$script:ActiveSessions = [System.Collections.Concurrent.ConcurrentDictionary[string, object]]::new()
...
function Initialize-PerformanceTracking {
    ...
    try {
        ...
        if ($Context -and $Context.PSObject.Properties['Config'] -and 
            ...
            $script:PerformanceConfig.Enabled = if ($perfConfig.enabled -ne $null) { $perfConfig.enabled } else { $true }
            $script:PerformanceConfig.DetailedLogging = if ($perfConfig.detailedLogging -ne $null) { $perfConfig.detailedLogging } else { $false }
            $script:PerformanceConfig.CollectSystemMetrics = if ($perfConfig.collectSystemMetrics -ne $null) { $perfConfig.collectSystemMetrics } else { $true }
            $script:PerformanceConfig.MaxHistoryEntries = if ($perfConfig.maxHistoryEntries -ne $null) { $perfConfig.maxHistoryEntries } else { 1000 }
            $script:PerformanceConfig.AutoExportThreshold = if ($perfConfig.autoExportThreshold -ne $null) { $perfConfig.autoExportThreshold } else { 500 }
        }
        ...
        Write-Host "Error in function 'Initialize-PerformanceTracking': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
function Measure-Operation {
    ...
    .EXAMPLE
        ...
        } -OperationName "ActiveDirectoryUserDiscovery" -Context $context
    .EXAMPLE
        $result = Measure-Operation -Operation {
            Invoke-GraphRequest -Uri "users" -Method GET
        } -OperationName "GraphUserQuery" -Context $context -Data @{
            TenantId = $tenantId
            RequestUri = "users"
        }
    #>
    ...
    try {
        ...
        if (Get-Command Write-MandALog -ErrorAction SilentlyContinue) {
            Write-MandALog -Message "Starting performance measurement for: $OperationName" -Level "DEBUG" -Component "PerformanceMetrics" -Data @{
                ExecutionId = $executionId
                Category = $Category
                CollectSystemMetrics = $shouldCollectSystemMetrics
            } -Context $Context
        }
...
function Calculate-SystemMetricsDelta {
    param(
        [Parameter(Mandatory=$true)]
        [hashtable]$Initial,
    ...
    try {
        ...
        Write-Host "Error in function 'Calculate-SystemMetricsDelta': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
function Add-PerformanceMetric {
    ...
    try {
        ...
        # Store the data
        $script:PerformanceData[$key] = $PerformanceData
        
        # Check if we need to auto-export due to threshold
...
function Start-PerformanceSession {
    param(
        [Parameter(Mandatory=$true)]
        [string]$SessionName,
    ...
    try {
        ...
        return $sessionData.ExecutionId
    } catch {
        Write-Host "Error in function 'Start-PerformanceSession': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}

function Stop-PerformanceSession {
    param(
        [Parameter(Mandatory=$true)]
        [string]$SessionName,
    ...
    try {
        ...
        if (Get-Command Write-MandALog -ErrorAction SilentlyContinue) {
            Write-MandALog -Message "Stopped performance session: $SessionName" -Level "SUCCESS" -Component "PerformanceMetrics" -Data @{
                ...
                FailedOperations = $sessionData.FailedOperations
            } -Context $Context
        }
        
        return $sessionData
    } catch {
        Write-Host "Error in function 'Stop-PerformanceSession': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
function Get-PerformanceReport {
    ...
    try {
        ...
        Write-Host "Error in function 'Get-PerformanceReport': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
function Clear-PerformanceData {
    ...
    try {
        ...
        Write-Host "Error in function 'Clear-PerformanceData': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
function Get-PerformanceStatistics {
    ...
    try {
        return @{
            ...
            Configuration = $script:PerformanceConfig.Clone()
            MemoryUsage = @{
            ...
            NewestMetric = if ($script:PerformanceData.Count -gt 0) {
                ($script:PerformanceData.Values | Sort-Object StartTime -Descending | Select-Object -First 1).StartTime
            } else { $null }
        }
    } catch {
        Write-Host "Error in function 'Get-PerformanceStatistics': $($_.Exception.Message)" -ForegroundColor Red
        throw
    }
}
...
if (-not (Get-Command Get-OrElse -ErrorAction SilentlyContinue)) {
    function Get-OrElse {
        ...
        try {
            ...
            Write-Host "Error in function 'Get-OrElse': $($_.Exception.Message)" -ForegroundColor Red
            throw
        }
    }
}
...
Export-ModuleMember -Function @(
    'Measure-Operation',
    'Start-PerformanceSession',
    'Stop-PerformanceSession',
    'Get-PerformanceReport',
...
Write-Host "[PerformanceMetrics.psm1] Module loaded successfully. Performance tracking enabled: $($script:PerformanceConfig.Enabled)" -ForegroundColor Green
