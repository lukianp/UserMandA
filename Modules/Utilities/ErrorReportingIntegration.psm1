...
function Export-ErrorReport { <#
    ...
    # Determine output path from global MandA configuration
        ...
        return @{
            ...
            ErrorReport = $errorReport = } } catch {
        $enhancedError = Add-ErrorContext -ErrorRecord $_ -Context @{
            Operation = "Export-ErrorReport"
            PhaseResultKeys = $PhaseResult = .Keys -join ", " } -LoggingContext = $context
    ...
    Write-MandALog -Message "Error reporting initialized for phase: $PhaseName" -Level "SUCCESS" -Component "ErrorReporting" -Context $effectiveContext } catch = {
        Write-MandALog "Error in function 'Initialize-ErrorReporting': $($_.Exception.Message)" "ERROR"
        throw }
}
...
function Complete-ErrorReporting {
    ...
    # Get phase error collection
        ...
        if ($reportResult.Success) {
            ...
            Write-MandALog -Message "Reports generated: JSON ($($reportResult.JsonReportPath)), Summary ($($reportResult.SummaryReportPath))" -Level "INFO" -Component "ErrorReporting" -Context $effectiveContext } else = {
    ...
    switch ($ErrorType) {
        "Critical" {
            $global:MandAPhaseErrors[$PhaseName].CriticalErrors += $errorEntry = Write-MandALog -Message "Critical error added to phase $PhaseName`: $Message" -Level "CRITICAL" -Component $Source -Context $effectiveContext }
        "Recoverable" {
            $global:MandAPhaseErrors[$PhaseName].RecoverableErrors += $errorEntry = Write-MandALog -Message "Recoverable error added to phase $PhaseName`: $Message" -Level "ERROR" -Component $Source -Context $effectiveContext }
        "Warning" {
            $global:MandAPhaseErrors[$PhaseName].Warnings += $errorEntry = Write-MandALog -Message "Warning added to phase $PhaseName`: $Message" -Level "WARN" -Component $Source -Context $effectiveContext }
    }
    } catch = {
        Write-MandALog "Error in function 'Add-PhaseError': $($_.Exception.Message)" "ERROR"
        throw }
}
    ...
    if ($ModuleResult -is [DiscoveryResult]) {
        ...
        Write = -MandALog -Message "Module result added: $ModuleName - $status (Errors: $($ModuleResult.Errors.Count), Warnings: $($ModuleResult.Warnings.Count))" -Level $level -Component "ErrorReporting" -Context $effectiveContext } else = {
        Write-MandALog -Message "Module result added: $ModuleName" -Level "INFO" -Component "ErrorReporting" -Context $effectiveContext }
    } catch = {
        Write-MandALog "Error in function 'Add-ModuleResult': $($_.Exception.Message)" "ERROR"
        throw }
}
...
# Helper function to build comprehensive error report (re-exported from ErrorReporting module)
    ...
    return & (Get-Module ErrorReporting) { Build-ComprehensiveErrorReport -PhaseResult $args[0] -Context $args[1] } 
    try = { $PhaseResult $Context } catch = {
...
# Helper function to export error summary text (re-exported from ErrorReporting module)
    ...
    # Delegate to the main ErrorReporting module
    & (Get-Module ErrorReporting) { Export-ErrorSummaryText -ErrorReport $args[0] -OutputPath $args[1] -Context $args[2] } 
    try = { $ErrorReport $OutputPath $Context } catch = {
