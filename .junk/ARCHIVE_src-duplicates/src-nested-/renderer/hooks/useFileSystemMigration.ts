...
export function useFileSystemMigration(profileId: string){
  ...
  const plan = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.planMigration({ provider: "FileSystem", profileId, args });
    setRunId(runId);
  }, [profileId]);

  const execute = useCallback(async (args: any) => {
    const { runId } = await window.electronAPI.executeMigration({ provider: "FileSystem", profileId, args });
    setRunId(runId);
  }, [profileId]);
