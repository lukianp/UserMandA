...
export function useOneDriveDiscovery(profileId: string): UseDiscoveryResult {
  const discovery = useDiscovery("OneDrive", profileId);
  ...
  const start = useCallback(async (args: Record<string, unknown> = {}) => {
    try {
      ...
      setError(err.message || "Failed to start OneDrive discovery");
    }
  }, [discovery]);
