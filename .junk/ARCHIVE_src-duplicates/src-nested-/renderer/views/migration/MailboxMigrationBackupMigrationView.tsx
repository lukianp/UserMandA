...
import { useMailboxMigrationBackupMigration } from "../../hooks/useMailboxMigrationBackupMigration";
import DataTable from "../../components/DataTable";

export default function MailboxMigrationBackupMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useMailboxMigrationBackupMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="mailbox-migration-backup-migration-view" data-cy="mailbox-migration-backup-migration-view">
      <h1 className="text-xl font-semibold">MailboxMigrationBackup Migration</h1>
