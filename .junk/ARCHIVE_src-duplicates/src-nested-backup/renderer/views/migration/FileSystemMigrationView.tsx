...
import { useFileSystemMigration } from "../../hooks/useFileSystemMigration";
import DataTable from "../../components/DataTable";

export default function FileSystemMigrationView(){
  const { selectedSourceProfile, selectedTargetProfile } = useProfileStore();
  const { plan, execute, progress, rows } = useFileSystemMigration(selectedTargetProfile?.id || selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="file-system-migration-view" data-cy="file-system-migration-view">
      <h1 className="text-xl font-semibold">FileSystem Migration</h1>
