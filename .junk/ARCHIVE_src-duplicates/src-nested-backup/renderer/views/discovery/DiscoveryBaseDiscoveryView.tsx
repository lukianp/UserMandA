...
import { useDiscoveryBaseDiscovery } from "../../hooks/useDiscoveryBaseDiscovery";
import DataTable from "../../components/DataTable";

export default function DiscoveryBaseDiscoveryView(){
  const { selectedSourceProfile } = useProfileStore();
  const { start, progress, rows } = useDiscoveryBaseDiscovery(selectedSourceProfile?.id || "");
  const [config, setConfig] = useState<Record<string, any>>({});

  return (
    <div className="p-4 space-y-3" data-testid="discovery-base-discovery-view" data-cy="discovery-base-discovery-view">
      <h1 className="text-xl font-semibold">DiscoveryBase Discovery</h1>
